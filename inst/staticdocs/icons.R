...
geom_Xisoprop <- sd_icon({
  x = .emptyPlot() + geom_Tisoprop(value=c(.25,.5,.75),color='magenta',size=0.5)
  ggplotGrob(x)
})

geom_smooth_tern <- sd_icon({
  data(Feldspar)
  x = .emptyPlot(data=Feldspar,aes(Ab,An,Or)) + geom_smooth_tern()
  ggplotGrob(x)
})

geom_Xline <- sd_icon({
  x = .emptyPlot() + geom_Lline(Lintercept=c(.25,.5,.75),color='orange',size=0.5)
  ggplotGrob(x)
})
